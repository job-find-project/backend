...
public class RegistrationController {
    ...
    public ResponseEntity<String> registerUser(@Valid @RequestBody UserDto userDto,
        ...
        if(!userDto.getPassword().equals(userDto.getConfirmPassword())) {
            return ResponseEntity.badRequest().contentType(MediaType.TEXT_PLAIN).body("Vui lòng nhập đúng mật khẩu");
        }
        if(result.hasErrors()) {
        ...
        if(user == null)
            return ResponseEntity.badRequest().contentType(MediaType.TEXT_PLAIN).body("Email đã được sử dụng");
        publisher.publishEvent(new RegistrationCompleteEvent(user, ApplicationUrl.getUrl(request)));
        return ResponseEntity
                .status(HttpStatus.OK)
                .contentType(MediaType.TEXT_PLAIN)
                .body("Đăng ký tài khoản thành công, vào email để xác thực tài khoản");
    }
    ...
    public ResponseEntity<?> verifyEmail(@RequestParam String token) {
        ...
        if(verificationTokenOptional.isPresent() && verificationTokenOptional.get().getUser().getIsEnabled()) {
            return ResponseEntity.status(HttpStatus.OK).contentType(MediaType.TEXT_PLAIN).body("Tài khoản đã được xác thực");
        }
        ...
        switch (result.toLowerCase()) {
            ...
            case "invalid":
                return ResponseEntity.badRequest().contentType(MediaType.TEXT_PLAIN).body("Xác thực không hợp lệ");
            default:
                return ResponseEntity.badRequest().contentType(MediaType.TEXT_PLAIN).body("Token hết thời hạn");
        }
    }
